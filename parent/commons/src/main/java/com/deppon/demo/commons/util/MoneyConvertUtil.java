...
public class MoneyConvertUtil {
	public static final String[] chineseDigits = new String[] { "零", "壹", "贰", "叁", "肆", "伍", "陆", "柒", "捌", "玖" };
	...
	public static String amountToChinese(double amount) {
			...
			if (i != 0) {
				if (i % 2 == 0)
					...
					if ("".equals(partChinese) && !beforeWanIsZero)
						// 如果“万”对应的part 为0，而“万”下面一级不为0，则不加“万”，而加“零”
						chineseStr.insert(0, "零");
					else {
						if (parts[i - 1] < 1000 && parts[i - 1] > 0)
							// 如果"万"的部分不为0,而"万"前面的部分小于1000大于 0，则万后面应该跟“零”
							chineseStr.insert(0, "零");
		...
		if ("".equals(chineseStr.toString())) // 整数部分为 0, 则表达为"零元"
		...
		if (numFen == 0 && numJiao == 0) {
			...
			chineseStr.append(chineseDigits[numJiao] + "角");
		} else { // “分”数不为 0
			if (numJiao == 0)
				chineseStr.append("零" + chineseDigits[numFen] + "分");
			else
				chineseStr.append(chineseDigits[numJiao] + "角" + chineseDigits[numFen] + "分");
		}
	...
	private static String partTranslate(int amountPart) {
		...
		for (int i = 0; i < amountStrLength; i++) {
			...
			if (digit == 0) { // 取到的数字为 0
				...
				chineseStr.insert(0, chineseDigits[digit] + units[i]);
				lastIsZero = false;
			}
			temp = temp / 10;
		}
